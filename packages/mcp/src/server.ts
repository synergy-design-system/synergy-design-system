...
export const createServer = () => {
  ...
  Object.values(prompts).forEach(prompt => prompt(server));
  Object.values(tools).forEach(tool => tool(server));
  Object.values(resources).forEach(resource => resource(server));

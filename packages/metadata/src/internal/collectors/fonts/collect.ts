/**
 * Collector for fonts package-level setup metadata.
 */
...
export const collect = async (
  config: FontsConfig,
  ctx: Context,
): Promise<Result<FontsRaw, CollectError>> => {
  ctx.logger?.info('FontsCollector: collecting package-level metadata');
  ...
  try {
    ...
    return {
      error: createCollectError('Failed to collect fonts package metadata', 'fonts', {

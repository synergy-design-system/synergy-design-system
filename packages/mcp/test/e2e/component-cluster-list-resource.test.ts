...
describe('component-cluster-list resource', () => {
  before(async () => {
  ...
  it('returns a flat array of component clusters', async () => {
    ...
    const clusters = JSON.parse(content.text) as unknown;
    assert.ok(Array.isArray(clusters), 'Expected an array');
    assert.ok(clusters.length > 0, 'Expected at least one cluster');
  });
});

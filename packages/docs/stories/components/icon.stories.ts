...
/**
 * ## Libraries
 * You can register additional icons to use with the <syn-icon> component through icon libraries. Icon files can exist locally or on a CORS-enabled endpoint (e.g. a CDN). There is no limit to how many icon libraries you can register and there is no cost associated with registering them, as individual icons are only requested when they're used.
 ...
 *
 * The default base path of the icon library is set to __assets/icons/__. To make the <syn-icon> work out of the box, without configuring anything, the used icons can be copied to this path in you application. This can either be done manually or with the help of the bundler.
 *
 * #### Angular + Webpack
 * Including assets from another library can be achieved in angular via configuring the assets configuration in the angular.json file. For more information have a look at the [angular documentation](https://angular.io/guide/workspace-config#asset-config).
 *
 ...
 * Here`s an example with adapted vite.config.ts that copies the Synergy icons to the path __assets/icons/__ of an vite project:
 *
 *  ```js
 ...
 * ```
 *
 * #### Custom icon library provided via CDN
 *
 ...
 * Check out the example above or the [Shoelace Docs](https://shoelace.style/components/icon?id=icon-libraries)
 * to see how to handle this.
 *
 *
 * ### Load all available icons via a bundled file
 ...
 *  resolver: (name: keyof typeof defaultIcons) => {
 *    if (name in defaultIcons) {
 *      return `data:image/svg+xml,${encodeURIComponent(defaultIcons[name])}`;
 *    }
 *    return '';
 *  },
 *  mutator: svg => svg.setAttribute('fill', 'currentColor'),

import { copyFile, glob, mkdir } from 'fs/promises';
...
export const runCopyModules = job('Copying submodules to dist...', async () => {
  ...
  const srcModules = await Array.fromAsync(
    glob('./src/**/*.css', { exclude: ['./src/**/index.css'] }),
  );
  ...
  return Promise.all(
    srcModules.map((src, index) => copyFile(src, destinationFileNames[index])),
  );
});

  ...
  render: () => html`
    <section>
      <article>
        ...
        <p>
          By default, you will have to wrap each element that needs a tooltip with a separate <code data-tooltip="something is in there">&lt;syn-tooltip&gt;</code> component.
          However, you may want to use a single tooltip instance for multiple elements.
          You can do so by using the <code>anchor</code> private attribute on any element that should trigger a tooltip and setup event listeners to update the tooltip content and anchor based on user interactions.
          The content of the tooltip will be taken from the value of the <code data-tooltip="This is just an example. You may draw the content from any attribute or property of the element.">data-tooltip</code> attribute.
          Have a look at this stories source code for an example of how to set this up.
          This can be added to any element, including those added dynamically after the initial setup.         
        </p>

        <div style="display: flex; gap: 16px; flex-wrap: wrap;">
          <syn-button data-tooltip="This will add a new button to the story. The new button will have a dynamically generated Tooltip">Add a new button dynamically</syn-button>
          <syn-input data-tooltip="Custom Tooltip" placeholder="Hover me too"></syn-input>
        </div>
      </article>
    </section>
    ...
    if (demoButton instanceof HTMLElement && demoContainer instanceof HTMLElement) {
      demoButton.addEventListener('click', () => {
        cnt = cnt + 1;
        const newElement = document.createElement('syn-button');
        newElement.textContent = 'Dynamic Button#' + cnt;
        newElement.setAttribute('data-tooltip', 'Tooltip for dynamic button#' + cnt);

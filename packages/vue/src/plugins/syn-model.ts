...
const createSynModelDirective = (
  bindingsMap: SynBindingsMap,
): ObjectDirective<SynBindableElement, SynBindingValue> => {
  const applyDirective = (
    el: SynBindableElement,
    binding: DirectiveBinding<SynBindingValue>,
  ) => {
    removeListener(el);
...
const createSynFormModelDirective = (
  bindingsMap: SynBindingsMap,
): ObjectDirective<SynFormBoundElement, SynFormModel> => {
  const applyDirective = (
    el: SynFormBoundElement,
    binding: DirectiveBinding<SynFormModel>,
  ) => {
    ...
    const controls = Array.from(
      el.querySelectorAll<SynNamedControl>('*[name]'),
    );
...
export const createSynModelPlugin = (
  ...
  install(app: App) {
    ...
    app.directive(
      directiveName,
      createSynModelDirective(bindings) as Directive,
    );
    app.directive(
      formDirectiveName,
      createSynFormModelDirective(bindings) as Directive,
    );
  },
});

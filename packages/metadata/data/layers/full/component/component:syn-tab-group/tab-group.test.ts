...
import type { SynTabShowEvent } from '../../events/syn-tab-show.js';
...
const waitForScrollButtonsToBeRendered = async (tabGroup: SynTabGroup): Promise<void> => {
  await waitUntil(() => {
    const scrollButtons = tabGroup.shadowRoot?.querySelectorAll('syn-icon-button');
    return scrollButtons?.length === 2;
  });
};

const getClientRectangles = (tabGroup: SynTabGroup): ClientRectangles => {
...
const expectHeaderToBeVisible = (container: HTMLElement, dataTestId: string): void => {
  const generalHeader = queryByTestId<SynTab>(container, dataTestId);
...
const expectOnlyOneTabPanelToBeActive = async (container: HTMLElement, dataTestIdOfActiveTab: string) => {
  await waitUntil(() => {
    const tabPanels = Array.from(container.getElementsByTagName('syn-tab-panel'));
    const activeTabPanels = tabPanels.filter((element: SynTabPanel) => element.hasAttribute('active'));
...
const expectPromiseToHaveName = async (showEventPromise: Promise<SynTabShowEvent>, expectedName: string) => {
  const showEvent = await showEventPromise;
  expect(showEvent.detail.name).to.equal(expectedName);
};

const waitForHeaderToBeActive = async (container: HTMLElement, headerTestId: string): Promise<SynTab> => {
  const generalHeader = queryByTestId<SynTab>(container, headerTestId);
  await waitUntil(() => {
...
describe('<syn-tab-group>', () => {
  ...
  it('displays all tabs', async () => {
    const tabGroup = await fixture<SynTabGroup>(html`
      <syn-tab-group>
        <syn-tab slot="nav" panel="general" data-testid="general-tab-header">General</syn-tab>
        <syn-tab slot="nav" panel="disabled" disabled data-testid="disabled-tab-header">Disabled</syn-tab>
        <syn-tab-panel name="general">This is the general tab panel.</syn-tab-panel>
  ...
  it('shows the first tab to be active by default', async () => {
    const tabGroup = await fixture<SynTabGroup>(html`
      <syn-tab-group>
        ...
        <syn-tab-panel name="general" data-testid="general-tab-content">This is the general tab panel.</syn-tab-panel>
  ...
  describe('scrolling behavior', () => {
    const generateTabs = (n: number): HTMLTemplateResult[] => {
      ...
      for (let i = 0; i < n; i++) {
        result.push(
          html`<syn-tab slot="nav" panel="tab-${i}">Tab ${i}</syn-tab>
            <syn-tab-panel name="tab-${i}">Content of tab ${i}0</syn-tab-panel> `
        );
      }
      return result;
    };
    ...
    it('shows scroll buttons on too many tabs', async () => {
      const tabGroup = await fixture<SynTabGroup>(html`<syn-tab-group> ${generateTabs(30)} </syn-tab-group>`);
    ...
    it('does not show scroll buttons if all tabs fit on the screen', async () => {
      const tabGroup = await fixture<SynTabGroup>(html`<syn-tab-group> ${generateTabs(2)} </syn-tab-group>`);
    ...
    it.skip('does not show scroll buttons if placement is start', async () => {
      const tabGroup = await fixture<SynTabGroup>(html`<syn-tab-group> ${generateTabs(50)} </syn-tab-group>`);
      tabGroup.placement = 'start';
    ...
    it.skip('does scroll on scroll button click', async () => {
      ...
      const tabGroup = await fixture<SynTabGroup>(
        html`<syn-tab-group> ${generateTabs(numberOfElements)} </syn-tab-group>`
      );
      ...
      const scrollToRightButton = tabGroup.shadowRoot?.querySelector('syn-icon-button[part*="scroll-button--end"]');
  ...
  describe('tab selection', () => {
    const expectCustomTabToBeActiveAfter = async (tabGroup: SynTabGroup, action: () => Promise<void>): Promise<void> => {
    ...
    const expectGeneralTabToBeStillActiveAfter = async (
      ...
      oneEvent(tabGroup, 'syn-tab-show').then(() => (showEventFired = true));
      oneEvent(tabGroup, 'syn-tab-hide').then(() => (hideEventFired = true));
      await action();
    ...
    it('does not change if a disabled tab is clicked', async () => {
      const tabGroup = await fixture<SynTabGroup>(html`
        <syn-tab-group>
          <syn-tab slot="nav" panel="general" data-testid="general-header">General</syn-tab>
          <syn-tab slot="nav" panel="disabled" data-testid="disabled-header" disabled>disabled</syn-tab>
          <syn-tab-panel name="general" data-testid="general-tab-content">This is the general tab panel.</syn-tab-panel>
    ...
    it('does not allow selection of disabled tabs with arrow keys', async () => {
      const tabGroup = await fixture<SynTabGroup>(html`
        <syn-tab-group>
          ...
          <syn-tab-panel name="general" data-testid="general-tab-content">This is the general tab panel.</syn-tab-panel>
          <syn-tab-panel name="disabled">This is the custom tab panel.</syn-tab-panel>
        </syn-tab-group>
      `);

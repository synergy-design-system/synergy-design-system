...
> If the component was initially rendered with the default setting, it will be updated to the new default setting.
...
```typescript
import { type SynDefaultSettingsChangedEvent } from '@synergy-design-system/components';
...
{Object
  ...
  .map(c => {
    ...
    return (
      <syn-details summary={c} key={`${c}--details`}>
        <ul>
          {Object
            .entries(settings)
            .map(([key, value]) => (
              <li>
                <strong key={`${c}--${key}--key`}>{key}</strong>: {value}
              </li>
            ))
          }
        </ul>

...
export default {
  ...
  plugins: [
    ...
    // With the changed combobox because of #627, we have also to adjust the custom-elements-manifest here.
    // The custom-elements.json generation needs to be updated, so the framework wrappers are created correctly again and the properties table of storybook documentation is correct.
    {
      ...
      packageLinkPhase({ customElementsManifest }){
        ['combobox', 'select'].forEach(component => {
          const module = customElementsManifest?.modules?.find(mod =>  mod.path.endsWith(`${component}.js`)).declarations[0];
          const valueMember = module.members.find(declaration => declaration.name === 'value');
          const defaultValueAttribute = module.attributes.find(attribute => attribute.fieldName === 'defaultValue');

          // Create an attribute entry for "value"
          ...
          module.attributes.push(valueAttribue);

          // Remove the defaultValue from attributes to have the same state as before the shoelace changes
          const defaultValueIndex = module.attributes.indexOf(defaultValueAttribute);
          module.attributes.splice(defaultValueIndex, 1);
        });
      },
    },

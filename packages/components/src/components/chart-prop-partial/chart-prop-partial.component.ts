 ...
 */
export default class SynChartPropPartial extends SynergyElement {
  ...
  private createSeriesOption(data: ChartDataInput): RegisteredSeriesOption[SupportedSeriesType] {
    switch (this.chartType) {
      ...
      case 'scatter':
        return {
          data: data as SeriesDataByType<'scatter'>,
          type: 'scatter',
        };
      case 'line':
      default:
  ...
  private buildConveniencePropertyConfig(): ECConfig {
    ...
    if (seriesOptions.length > 0) propertyConfig.series = seriesOptions;
  ...
  private applyConvenienceUpdatesToChart(changedProperties: PropertyValues<this>): void {
    if (!this.chartInstance) return;

    const convenienceUpdate: ECConfig = {};

    if (changedProperties.has('xAxisData') && this.xAxisData !== undefined) {
      convenienceUpdate.xAxis = this.xAxisData;
    }

    if (changedProperties.has('yAxisData') && this.yAxisData !== undefined) {
      convenienceUpdate.yAxis = this.yAxisData;
    }

    if (changedProperties.has('chartType') || changedProperties.has('chartData')) {
      convenienceUpdate.series = this.buildSeriesOptionsFromData();
    }

    if (Object.keys(convenienceUpdate).length === 0) return;

    this.chartInstance.setOption(convenienceUpdate);
    this.applyPalette();
  }
  ...
  protected updated(changedProperties: PropertyValues<this>): void {
    ...
    const convenienceChange = changedProperties.has('chartType')
      || changedProperties.has('chartData')
      || changedProperties.has('xAxisData')
      || changedProperties.has('yAxisData');

    if (convenienceChange && this.chartInstance) {
      this.applyConvenienceUpdatesToChart(changedProperties);
      return;
    }

...
describe('<syn-icon-button>', () => {
  ...
  describe('when icon attributes are present', () => {
    it('renders an syn-icon from a library', async () => {
      const el = await fixture<SynIconButton>(html` <syn-icon-button library="system" name="check"></syn-icon-button> `);
      expect(el.shadowRoot?.querySelector('syn-icon')).to.exist;
    });

    it('renders an syn-icon from a src', async () => {
      const fakeId = 'test-src';
      const el = await fixture<SynIconButton>(html` <syn-icon-button></syn-icon-button> `);

      el.src = `data:image/svg+xml,${encodeURIComponent(`<svg id="${fakeId}"></svg>`)}`;

      const internalSynIcon = el.shadowRoot?.querySelector('syn-icon');

      await waitUntil(() => internalSynIcon?.shadowRoot?.querySelector('svg'), 'SVG not rendered');

      expect(internalSynIcon).to.exist;
      expect(internalSynIcon?.shadowRoot?.querySelector('svg')).to.exist;
      expect(internalSynIcon?.shadowRoot?.querySelector('svg')?.getAttribute('id')).to.equal(fakeId);
    });
  });
  ...
  describe('when label is present', () => {
    it('the internal aria-label attribute is set to the provided label when rendering a button', async () => {
      ...
      const el = await fixture<SynIconButton>(html` <syn-icon-button label="${fakeLabel}"></syn-icon-button> `);
    ...
    it('the internal aria-label attribute is set to the provided label when rendering an anchor', async () => {
      const fakeLabel = 'some label';
      const el = await fixture<SynIconButton>(html`
        <syn-icon-button href="some/path" label="${fakeLabel}"></syn-icon-button>
      `);

...
describe('styles-list resource', () => {
  ...
  it('returns a flat array of styles', async () => {
    ...
    const styles = JSON.parse(content.text) as unknown;
    assert.ok(Array.isArray(styles), 'Expected an array');
    assert.ok(styles.length > 0, 'Expected at least one style');
  });
});

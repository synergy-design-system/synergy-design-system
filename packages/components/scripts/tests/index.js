...
const synRefreshComponentBundle = () => {
  ...
  const rebuildSynergy = async () => {
    ...
    await jobs.runCem('dist.testrunner');
    await jobs.runCreateStyles({ componentDistDir: temporaryOutDir, stylesDir: getPath('../src/styles') });
  ...
  return {
    ...
    serverStart: async ({ fileWatcher }) => {
      ...
      fileWatcher.add('./src/utilities/**/!(*.(test)).ts');

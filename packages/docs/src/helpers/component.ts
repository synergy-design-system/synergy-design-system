...
export const storybookDefaults = (customElementTag: string): any => {
  ...
  const getOptimizedArgTypes = () => {
    ...
    const output = {
      ...
      ...manifest?.members?.filter(member => member.kind === 'method' && !(member?.privacy === 'private')).reduce((acc: any, method: any) => {
      ...
      ...getProperties()?.reduce((acc: any, property: string) => {
        ...
        acc[property] = { control: false, table: { category: 'Properties' } };
        return acc;
      }, {}),
    };
...
export const generateStoryDescription = <T extends Component>(component: T, attribute: Attribute<T>) => ({
  story: (docsTokens?.components?.[component]?.[attribute] as AttributeDescription)?.description?.value ?? 'No Description',
});

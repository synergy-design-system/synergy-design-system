...
export const sortByEntityId = <T>(entities: MetadataEntity<T>[]): MetadataEntity<T>[] =>
...
export const layerExistsForEntity = <T>(entity: MetadataEntity<T>, layer: LayerName): boolean =>
  !!entity.layers?.[layer] && entity.layers[layer].length > 0;

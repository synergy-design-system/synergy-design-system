...
export type SynEventMap<TEvents extends SynEventTuple[]> = {
  [
    TEventTuple in TEvents[number] as TEventTuple[0] extends string
      ...
      : never
  ]?: (event: TEventTuple[1]) => unknown;
};

export type VueEmit<TEvents extends SynEventTuple[]> = EmitFn<{
  [TEventTuple in TEvents[number] as TEventTuple[0]]: (
    event: TEventTuple[1],
  ) => void;
}>;

export type SynNativeHTMLProps = AllowedComponentProps &
  ComponentCustomProps &
  VNodeProps;
...
> = {
  new (): {
    $props: Partial<SynTemplateProps<SynElementProps<TElement>>> &
      SynNativeHTMLProps;
...
export type SynIntrinsicElement<
  ...
  TEvents extends SynEventTuple[] = [],
> = HTMLAttributes &
  Partial<SynTemplateProps<SynElementProps<TElement>>> &
  SynEventMap<TEvents>;
